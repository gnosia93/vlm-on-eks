...
* 2단계 — 병렬 워커 (EC2 g7e 2대) 같은 워커 프로그램을 담은 인스턴스를 2대 띄웁니다. 유일한 차이는 환경변수 SHARD_INDEX(0 또는 1)뿐이에요. 각 워커는 매니페스트 전체를 읽되, 줄번호를 2로 나눈 나머지가 자기 인덱스와 같은 줄만 처리합니다. 0번은 짝수 줄, 1번은 홀수 줄. 이렇게 하면 서로 겹치지 않고 데이터가 정확히 반씩 나뉘고, 둘이 통신할 필요도 없습니다(완전 독립 병렬).

* 3단계 — 모델 로딩과 추론 InternVL3-78B는 GPU 하나에 안 올라가는 큰 모델이라, 한 인스턴스 안의 GPU 4장에 나눠 올립니다(텐서 병렬, TP=4). g7e는 GPU당 96GB라 여유가 충분해요. 워커는 시작 시 S3에서 가중치를 로컬 NVMe로 내려받아 vLLM으로 한 번만 로드한 뒤, 자기 담당 이미지들을 배치로 묶어 추론합니다.

* 4단계 — 결과 저장 (S3) 추론 결과는 각자 shards/shard-00000.jsonl, shard-00001.jsonl로 S3에 주기적으로 업로드됩니다. 중간중간 올리기 때문에 인스턴스가 중간에 죽어도 이미 한 건 안 날아가고, 다시 켜면 그 지점부터 이어서(resume) 처리합니다.

* 5단계 — 자동 종료 워커가 자기 몫을 다 끝내면 인스턴스가 스스로 종료됩니다. g7e는 비싸기 때문에, 놀면서 요금 나가는 걸 막는 안전장치예요.
...
```
# 입력 건수
aws s3 cp s3://my-vlm-data-bucket/input/manifest.jsonl - | wc -l
# 출력 건수
aws s3 cp s3://my-vlm-data-bucket/output/run-2026-07-22/train.jsonl - | wc -l
```
