...
영상 배치를 돌릴 때 "가중치만 겨우 올라가는" 구성은 KV 캐시가 부족해 배치가 제대로 돌지 않거나 OOM(메모리 부족)이 발생하므로 여유를 크게 잡아야 합니다. 
품질을 최우선으로 하여 BF16을 쓴다면 최소 320GB 의 VRAM 이 필요하고, 배치나 영상 길이를 키운다면 640GB 까지 확보하는 것이 안전합니다. 
품질과 효율의 균형을 맞추는 경우 FP8 양자화를 시도할 수도 있습니다. 이때 FP8 연산을 지원하는 Nvidia Hopper(H100, H200), Blackwell(B200) 또는 Ada Lovelace(L40S) 아키텍처를 채용한 GPU 를 선택하는 것이 유리합니다. 
...
* s3 에 저장된 전체 영상 프레임에 대한 배치 인퍼런스 작성 -> 필요한 경우 k8s job 으로 실행.
