...
* 학습 프레임워크 - HuggingFace transformers + peft(LoRA) 직접 구현. 스펙에 명시한 "16프레임 × 448×448 × max_num=1 (프레임당 타일 1개, 4096 토큰)
* 학습 시 데이터 위치 - S3의 finevideo/sports/ 아래(manifest / frames / inference)를 학습 인스턴스 로컬로 먼저 sync해두고, 로컬 경로로 읽음. 
* 라벨 선택 규칙 - "영상당 최신 1개만" 
* GPU 구성 - 단일 노드(GPU 1~8장). 1B라 LoRA는 가볍고, torchrun으로 단일노드 멀티GPU 돌아가되 GPU 1장으로도 실행 가능하게.
* InternVL3-1B 가중치 - hf download OpenGVLab/InternVL3-1B로 받아 로컬에 있음.
* 학습 대상(LoRA 붙이는 위치) - 언어모델(LLM)의 attention/MLP 선형층에 LoRA, 비전 인코더는 freeze. distillation에서 흔한 설정이고 1B에 적합.
* teacher answer가 **자유 형식 한국어 문장(캡션)** 이다. 그래서 학습은 "프레임 16장 + 프롬프트 → 캡션 생성"을 **SFT(지도 파인튜닝)**로 하는 구조임.
* 손실은 answer 토큰에만 걸고(프롬프트·이미지 토큰은 -100 마스킹), 표준 causal LM loss 적용.
* 로짓 distillation이 아니라 teacher 출력을 정답으로 쓰는 sequence-level distillation.
