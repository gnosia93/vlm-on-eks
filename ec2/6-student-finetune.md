...
| GPU | 가능한 세팅 |
| :--- | :--- |
| **16 GB** <br>*(T4, V100)* | `batch=1` + `gradient_checkpointing` + `gradient_accumulation`으로 유효 배치 확보 |
| **24 GB** <br>*(RTX 3090/4090, A10)* | `batch=1~2`, 여유 있게 학습 가능 ✅ **워크샵 권장** |
| **40 GB+** <br>*(A100)* | `batch=4` 이상, checkpointing 없이도 쾌적 |
  

아래 기법을 사용하여 훈련시 필요한 GPU 메모리를 줄일 수 있다.  

* gradient_checkpointing=True — 활성값을 저장 대신 재계산. 메모리 30~40%↓, 속도 약 20%↓. 16GB에서는 사실상 필수.
* 프레임 수 축소 — 16 → 8프레임이면 이미지 토큰이 2,048로 절반. 활성값이 크게 줄지만 시간 정보도 줄어드니 정확도와 trade-off.
* batch=1 + gradient accumulation — 물리 배치는 1로 두고 accumulation_steps로 유효 배치를 키워, 메모리는 그대로 두고 학습 안정성만 확보.

_student가 1B라 가중치·옵티마이저는 3 GB 안쪽으로 가볍고, 진짜 변수는 16프레임(4K 이미지 토큰)에서 오는 활성값이다. 24GB GPU면 batch=1~2로 무난하며, 16 GB라면 gradient_checkpointing을 켜면 충분히 돌아간다._
