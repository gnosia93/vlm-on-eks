...
def main():
    ...
    for shard in range(N_SHARDS):
        ...
        done_key = f"{PREFIX}/_done/shard-{shard:05d}"
        if s3_exists(done_key):
            print(f"[shard {shard:05d}] already done, skip")
            continue

        fn = f"data/train-{shard:05d}-of-01357.parquet"
        ...
        for idx, row in enumerate(df.iter_rows(named=True)):
            vid = safe_id(row["fname"], shard, idx)
            base = f"{PREFIX}/{vid}"
        ...
        print(f"[shard {shard:05d}] sports {n}개 업로드 (누적 {total})")
    ...
    # 매니페스트
    manifest = {"category": CATEGORY, "prefix": PREFIX, "total_videos": total}
